...
// Version: 2.8.9 (Fixed non-functional Midterm/Note cards)
// =================================================================================
...
function runApp() {
    ...
    const handleLogout = () => {
        ...
        setButtonState(dom.login.btn, dom.login.btnText, dom.login.loader, false, "Login");
        // No need to clear the interval, it should run globally
    };
    ...
    const renderCtView = (subjectCode) => {
        ...
        const midtermCard = createSpecialCard('midterm-card', '📋', 'Midterm', pdfLinks[subjectCode]['Midterm'], () => renderPdfView(`${subjectCode} - Midterm`, pdfLinks[subjectCode]['Midterm']));
        dom.ct.midtermSection.appendChild(midtermCard);
        
        const classNoteCard = createSpecialCard('note-card', '🗒️', 'Class Notes', pdfLinks[subjectCode]['ClassNote'], () => renderPdfView(`${subjectCode} - Class Note`, pdfLinks[subjectCode]['ClassNote']));
        dom.ct.classNoteSection.appendChild(classNoteCard);
    ...
    const renderPdfView = (title, url) => {
        ...
        // The old screenshotShield logic is no longer needed due to the global security overlay
    };
    ...
    const checkDevTools = () => {
        ...
        dom.securityOverlay.classList.toggle('visible', isOpen);
    };
        ...
        document.addEventListener('keydown', event => {
            if (event.key === 'F12' || 
               ...
               (event.ctrlKey && event.key === 'S')) {
                ...
                dom.securityOverlay.classList.add('visible'); // Force show as an extra measure
            }
        });
        
        // Periodically check for dev tools being opened
        if (appState.devToolsCheckInterval) clearInterval(appState.devToolsCheckInterval);
        appState.devToolsCheckInterval = setInterval(checkDevTools, 500);
    };
    ...
    const init = async () => {
        addEventListeners();
        setupCosmicEffects();
        setupSecurity(); // <-- Initialize the new security features
        switchView(dom.login.section);
...
// The old profile icon logic is not needed as it's not in the HTML anymore
// window.toggleOptions and its listener are removed.
